        ...
        for(int j = 0 ; j < std::get<2>(current) ; j++){
            ...
            switch (std::get<0>(current)) {
				...
				case 3:
					...
					break;
                case 4:
                    EnemyGroup->AddNewObject(enemy = new ScreendoorEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
                    break;
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	const int y = my / BlockSize;
	if (button & 1) {
		if (mapState[y][x] != TILE_OCCUPIED || preview->remove) {
			if (!preview)
				return;
			// Check if valid.
			if (!CheckSpaceValid(x, y) && !preview->remove) {
			...
			// To keep responding when paused.
			preview->Update(0);
            if(!preview->remove) { // for RemoveTurret, don't occupy mapState
...
void PlayScene::AddLife(int life) {
    this->lives += life;
    UILives->Text = std::string("Life ") + std::to_string(this->lives);
}
void PlayScene::ReadMap() {
	...
	for (int i = 0; i < MapHeight; i++) {
		for (int j = 0; j < MapWidth; j++) {
            ...
            if(MapId == 1) {
                if(i % 2) {
                    TileMapGroup->AddNewObject(new Engine::Image("play/green.png", 0, i * BlockSize, BlockSize * 6, BlockSize));
                    TileMapGroup->AddNewObject(new Engine::Image("play/green.png", 6 * BlockSize, i * BlockSize, BlockSize * 6, BlockSize));
                } else {
                    TileMapGroup->AddNewObject(new Engine::Image("play/dark-green.png", 0, i * BlockSize, BlockSize * 6, BlockSize));
                    TileMapGroup->AddNewObject(new Engine::Image("play/dark-green.png", 6 * BlockSize, i * BlockSize, BlockSize * 6, BlockSize));
                }
            }
            else {
                if(i % 2) {
                    TileMapGroup->AddNewObject(new Engine::Image("play/night.png", 0, i * BlockSize, BlockSize * 6, BlockSize));
                    TileMapGroup->AddNewObject(new Engine::Image("play/night.png", 6 * BlockSize, i * BlockSize, BlockSize * 6, BlockSize));
                } else {
                    TileMapGroup->AddNewObject(new Engine::Image("play/dark-night.png", 0, i * BlockSize, BlockSize * 6, BlockSize));
                    TileMapGroup->AddNewObject(new Engine::Image("play/dark-night.png", 6 * BlockSize, i * BlockSize, BlockSize * 6, BlockSize));
                }
            }
		}
	}
...
void PlayScene::ConstructUI() {
	...
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/PeaShooter.png", 168, BlockSize*MapHeight + 5, 0, 0, 0, 0)
	...
	// Button 2
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/Puffshroom.png", 288, BlockSize*MapHeight, 0, 0, 0, 0)
    ...
    // Button 3
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/SnowPea.png", 408, BlockSize*MapHeight + 3, 0, 0, 0, 0)
        , 410, 128*MapHeight, SnowPeaTurret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 2));
    UIGroup->AddNewControlObject(btn);
    // Button 4
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/WallNut.png", 534, BlockSize*MapHeight + 5, 0, 0, 0, 0)
        , 530, 128*MapHeight, WallNutTurret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 3));
    UIGroup->AddNewControlObject(btn);
    // Button 5
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/ExplodeNut.png", 654, BlockSize*MapHeight + 5, 0, 0, 0, 0)
        , 650, 128*MapHeight, ExplodeNutTurret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 4));
    UIGroup->AddNewControlObject(btn);
    // Button 6
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/shovel.png", 774, BlockSize*MapHeight, 0, 0, 0, 0)
        , 770, 128*MapHeight, RemoveTurret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 5));
    UIGroup->AddNewControlObject(btn);
    // Button 7
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/Sunflower.png", 48, BlockSize*MapHeight, 0, 0, 0, 0)
        , 50, 128*MapHeight, SunflowerTurret::Price);
...
void PlayScene::UIBtnClicked(int id) {
    ...
    else if (id == 2 && money >= SnowPeaTurret::Price)
        preview = new SnowPeaTurret(0, 0);
    else if (id == 3 && money >= WallNutTurret::Price)
        preview = new WallNutTurret(0, 0);
    else if (id == 4 && money >= ExplodeNutTurret::Price)
        preview = new ExplodeNutTurret(0, 0);
    else if (id == 5 && money >= RemoveTurret::Price)
        preview = new RemoveTurret(0, 0);
    else if (id == 6 && money >= RemoveTurret::Price)
        preview = new SunflowerTurret(0, 0);
	if (!preview)
		return;
	preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
